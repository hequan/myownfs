									...
									void *data)
{
	...
	if (unlikely(IS_ERR(ret)))
		printk(KERN_ERR "Error mounting filesystem");
	else
...
static int simplefs_init(void)
{
	...
	if (likely(retval == 0))
		printk(KERN_INFO "Sucessfully register filesystem.\n");
	else
		printk(KERN_ERR "Failed to register filesytem. Error: [%d]", retval);
	return retval;
}

static int simplefs_exit(void)
{
	int retval;
	retval = unregister_filesystem(&simple_fs_type);
	if (likely(retval == 0))
		printk(KERN_INFO "Sucessfully unregister filesystem.\n");
	else
		printk(KERN_ERR "Failed to unregister filesytem. Error: [%d]", retval);
	return retval; 
}
